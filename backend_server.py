        ...
        async for message in websocket:
            try:
                ...
                if message_type == 'frame':
                    # Video frame message
                    required_fields = ['type', 'timestamp', 'source_id', 'frame_number', 'frame']
                    if not all(field in data for field in required_fields):
                        logger.warning(f"Invalid frame message from {device_id}: missing required fields")
                        continue
                    
                    logger.debug(f"Received video frame from {data.get('source_id')}, frame {data.get('frame_number')}")
                    
                elif message_type == 'detection':
                    # Detection event - validate required fields
                    required_fields = ['type', 'timestamp', 'source_id', 'current_count', 'frame_number', 'event_type']
                    if not all(field in data for field in required_fields):
                        logger.warning(f"Invalid detection message from {device_id}: missing required fields")
                        continue
                    
                    logger.debug(f"Received detection from {data.get('source_id')}: count={data.get('current_count')}")
                    
                else:
                    # Unknown message type - try to handle for backward compatibility
                    if 'frame' in data:
                        logger.debug(f"Received legacy frame message from {data.get('source_id')}")
                    elif 'current_count' in data:
                        logger.debug(f"Received legacy detection message from {data.get('source_id')}")
                    else:
                        logger.warning(f"Unknown message type '{message_type}' from {device_id}")
                        continue
                ...
                if dashboard_clients:
                    ...
                    logger.debug(f"Broadcasted {message_type} to {len(dashboard_clients)} dashboard clients")
                else:
                    logger.debug("No dashboard clients connected, message not broadcasted")
