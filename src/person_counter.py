...
Examples:
  ...
  python person_counter.py --ws-url ws://localhost:8001/ws
  ...
  python person_counter.py --config config.json
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="Output video file path (optional)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="WebSocket server URL (e.g., ws://localhost:8001/ws)",
    )
...
async def async_main():
    ...
    try:
        ...
        logger.info(
            f"  Output path: {args.output if args.output else 'None (display only)'}"
        )
        ...
        logger.info(
            f"  Confidence threshold: {system_config.video.confidence_threshold}"
        )
        ...
        detector = create_detector(
            onnx_model_path=(
                system_config.onnx.model_path if system_config.onnx.enable else None
            ),
        ...
        # Create tracker
        tracker = PersonTracker(max_distance=system_config.video.tracking_distance)
        ...
        if system_config.websocket.enable and args.enable_streaming:
            ...
            logger.info(
                f"Frame streaming enabled: {streaming_config.max_frame_rate} FPS, quality={streaming_config.jpeg_quality}"
            )
        ...
        final_count = await processor.process_video_async(
            input_source=input_source, output_path=args.output
        )
        ...
        if websocket_publisher:
            ...
            logger.info(
                f"Buffered frames: {websocket_publisher.get_frame_buffer_size()}"
            )
        logger.info("=" * 60)
